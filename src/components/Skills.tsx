        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, index) => {
            ...
            return (
              <div
                key={index}
                className={`card-glass rounded-2xl p-8 shadow-card hover:shadow-glow transition-all duration-500 group hover:scale-[1.02] ${
              ...
              >
                ...
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill, skillIndex) => (
                    <span
                      key={skillIndex}
                      className="px-4 py-2 bg-muted/50 hover:bg-primary/20 rounded-lg text-foreground/90 transition-all duration-300 hover:scale-105 cursor-default border border-border/50 hover:border-primary/50"
                    >
                      {skill}
                    </span>
                  ))}
